...
// The recorder a stage test is handed in place of the real one (06-testing Doubles). It
// keeps what the stage counted and writes no row, so a slice test can assert logic/16
// step 3's counters without a database or the queue behind them. The tests that need the
// row as well - the privacy sweep, the Usage totals - use `record` itself.
