...
export function ensureBaseline(deps: RevisionDeps, projectId: string): Promise<BaselineResult> {
  return withProjectControl(deps.db, projectId, () => adoptBaseline(deps, projectId));
}
...
export function adoptBaseline(
  ...
  return transact(deps.db, (): BaselineResult => {
    ...
    const recordedFingerprint = (key: string): string => {
      ...
      if (value === undefined) throw new Error(`No baseline fingerprint for retained work ${key}`);
      return value;
    };
