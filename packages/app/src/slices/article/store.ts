...
// The end-matter split runs "when the article becomes `done` or `provided`", so both the stage
// that writes an article and Play, which attaches a pasted one, reduce it the same way. The
// reduction lives here rather than in either caller because a rule that ran on one path and not
// the other is exactly the defect this fixes.
