...
for cli in codex claude gemini; do
  if command -v "$cli" >/dev/null 2>&1; then
    ...
    if [[ "$cli" == gemini ]]; then
      docker exec "$smoke_container" node -e \
        "fetch('http://127.0.0.1:6969/api/providers/gemini/models').then(r => r.json()).then(body => { if (!Array.isArray(body.models) || body.models.length < 2) process.exit(1) }).catch(() => process.exit(1))"
    fi
  fi
done
