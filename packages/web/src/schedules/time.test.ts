...
it("rejects nonexistent spring-forward time and uses the earlier repeated time", () => {
  ...
  expect(() => scheduleInstant("2027-03-28T02:30", "Europe/Berlin")).toThrow("does not exist");
  expect(() => scheduleInstant("2027-10-03T02:15", "Australia/Lord_Howe")).toThrow(
    "does not exist",
  );
  expect(() => scheduleInstant("2027-03-14T02:30", "America/New_York")).toThrow("does not exist");
