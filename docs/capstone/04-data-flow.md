...
- Network: only the three ports' adapters (`packages/app/src/adapters/**`) and `slices/telemetry/collector-client.ts`.
- Child processes: `adapters/llm/claude-code.ts`, `adapters/llm/codex.ts` (agent CLIs) and `slices/video/ffmpeg.ts`; all receive an `AbortSignal`.
- Filesystem: `slices/storage` (project folders, staging, downloads) and `kernel/log` (log files).
