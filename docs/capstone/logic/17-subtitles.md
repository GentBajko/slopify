...
5. Alignment acquires the per-model-cache process lock, lazily downloads/verifies pinned English weights, decodes each spoken segment to 16 kHz mono PCM and runs ONNX WASM in an abortable Node child. CTC alignment and speech-agreement checks reject mismatched transcripts. Relative word times become final-export times by adding segment and silence-gap offsets (`adapters/alignment/`, `slices/subtitles/prepare.ts`).
