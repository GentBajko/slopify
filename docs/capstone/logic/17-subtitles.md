...
4. Revision recipes separate `subtitles:timing`, `subtitles:cues`, and `subtitles:files`. Timing fingerprints include narration identities, spoken text, measured duration, silence gaps, the edge silence, language and alignment version. Font, size, position and frame changes affect file/style work without forcing timing alignment (`slices/rebuild/{recipe-audio,recipe-exports}.ts`). With captions Off, timing still runs when the YouTube description is on (its chapters read the word times) and no cue or caption file is made; hand-edited captions skip timing unless the description needs it (scenario 27).
5. Alignment acquires the per-model-cache process lock, lazily downloads/verifies pinned English weights, decodes each spoken segment to 16 kHz mono PCM and runs native ONNX Runtime (`onnxruntime-node`, CPU) in an abortable Node child: one session per job with `min(8, CPUs − 1)` intra-op threads, overridable with `SLOPIFY_SUBTITLE_THREADS` (`adapters/alignment/threads.ts`). Windows run one after another because each 12 s window starts at the previous window's last accepted word; inference is about 96% of alignment time, measured at about 30× realtime for 109 minutes of narration on a 32-thread machine (the old single-thread WASM runtime ran 4.5× on the same narration). Native logits are identical for any thread count but not bit-identical to WASM, so about 15% of word times differ from the WASM runtime, almost all by one 20 ms frame. If the native engine cannot load, the worker fails with a reinstall instruction; Intel Macs have no build and are told to turn subtitles off. CTC alignment and speech-agreement checks reject substantial mismatches. After a successfully aligned prefix, a short missing transcript passage can be skipped only when four following words supply a strong acoustic anchor (at least 20 letters, at most 20% recognition error, and each anchor word confidence at least 0.75). Recovery skips at most 40 words per event and at most 60 words or 5% of the segment transcript overall. It never guesses timestamps or inserts unspoken words; unmatched prefixes, extra speech, weak anchors and exhausted budgets remain failures. Relative word times become final-export times by adding segment, silence-gap and leading edge-silence offsets (`adapters/alignment/`, `slices/subtitles/prepare.ts`).
