...
2. Events: one install event; one event per project created; one event per stage completing: research, article, each intro/outro text, audio per segment (body, intro, outro), images, thumbnail, video, document. Each event carries its own ID, the machine ID, the app version, and its counters.
