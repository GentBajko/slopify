...
| Attempt | `kernel/runner/model.ts` | table `attempts` | one provider call attempt with error text (`logic/01` §Q4) |
...
| Entry | `slices/library/model.ts` | table `entries` | intro / outro, Text or LLM mode (`logic/15`, §Q98 of the logic interview) |
...
| Setting | `slices/settings/model.ts` | table `settings` | silence gap seconds, appearance (`logic/11` §Q99; uiux §Q19) |
| RunConfig | `slices/admission/model.ts` | column `projects.config` (JSON) | the full Play configuration, keyword values, rendered prompt texts (`logic/03` §Q25, `logic/04`) |
...
- Project: `id` ULID; `title` text ≤200; `format` enum `16:9`, `9:16`; `config` JSON (RunConfig); `status` derived, never stored (`logic/01` §Q9); `created_at`, `updated_at` UTC ISO-8601 text.
...
- ProviderKey: `provider` primary key, `key` text, `updated_at`. CLI providers have no row; their `installed` status is computed at request time (§Q10).
...
- Money and quantities: none are currency; durations in integer milliseconds; counts as integers (§Q19).

## Relationships

- Project 1-6 Stage (one per kind, created at Play per `logic/01` step 1); Stage 1-n Attempt; Stage 1-n StagePiece; Project 1-n Output; Project references Prompt and Entry names only inside `config` (never foreign keys: a deleted template must not touch a project, `logic/15` §Q123).
...
Three representations (§Q18, §Q19): SQLite rows (snake_case columns, JSON columns for config, slots, meta, payload) → domain objects (TypeScript types per slice `model.ts`, camelCase, enums as string literal unions) converted in each slice's `repo.ts`; domain → API JSON in `edge/http/<context>.ts` (same shapes, dates as ISO strings, derived `status` added on Project); API JSON → SPA types via the generated client (§Q27). Files never pass through the API as JSON; they are served from `/files/...`.

## Validation

- HTTP edge: every request body validated against a schema before a slice sees it (library per `stack`); violations → RFC 9457 400 (§Q18, §Q19).
- SQLite constraints: enums as CHECK constraints, uniqueness (`prompts(kind, lower(name))`, `entries(category, lower(name))`, `voices(provider, voice_id)`), foreign keys with cascade delete from `projects` to `stages`, `attempts`, `stage_pieces`, `outputs` (§Q19; `logic/14` §Q117).
...
Migrations in `packages/app/src/kernel/db/migrations/NNNN-<name>.sql`, forward-only, applied at boot in order, recorded in `schema_migrations(version, applied_at)`; the app refuses to start when the database records a version newer than it knows (§Q19). WAL journal mode on open (§Q25). Planned DDL, transcribed into the first migration when `build` runs:
...
Tables with no code model: `schema_migrations`. Collector schema (§Q5, `logic/16`): `events(id TEXT PRIMARY KEY, machine_id, type, payload, received_at)` and `aggregates(key TEXT PRIMARY KEY, value INTEGER)` in the managed database; the engine is `stack`'s.

Data lifecycle (§Q19; `logic/14`): hard deletes only; projects and templates kept until the user deletes them; the telemetry log kept forever; no archival, no legal hold; PII: none stored except provider keys, classified secret and never exported. Analytics path: local `telemetry_events` → collector aggregates; no warehouse. Search: none. Caching: none (model lists are fetched per Play load, `logic/02` §Q15). Backups: none for local data (§Q7 of §0.5, `logic/14`); collector: the host's daily backup (§Q24).
