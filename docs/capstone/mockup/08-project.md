...
implements: [Q1, Q9, Q13, Q17, Q18, Q19, Q21, Q25, Q28, Q30, Q31]
assumed:
...
The whole pipeline of one project, stage by stage, with its outputs and actions (Q1, Q17).
...
Element tree:
- App shell
  - Header: back, title (Q19), project status, format (Q18), created, prompts used (assumed), Cancel (Q17)
  - Stage 1 Research: notes text, download (Q17)
  - Stage 2 Article: text, edit, download, the sources and glossary files split from the end matter (Q17, Q31)
  - Stage 3 Audio: players for intro, body, outro when present, download each, re-run with another voice (Q17, Q30)
  - Stage 4 Images: grid per image prompt, thumbnail apart (Q13, Q17); per-image download and regenerate; download all; re-run stage
  - Stage 5 Video: player, download mp4, re-render (Q17)

## Elements

| Label | Does | Leads to |
|---|---|---|
| Cancel | Stops the running project (Q17) | Stays; stage statuses update |
| Download .txt (research) | Saves the research notes (Q1, Q17) | File |
| Edit (article) | Opens the text inline for editing (Q17) | Stays |
| Save & re-run from audio | Replaces the article and re-runs audio, images, video (Q17) | Stays |
| Discard | Drops edits | Stays |
| Download .txt (article) | Saves the article (Q1) | File |
| Sources, Glossary | Saves the end-matter files split out for narration (Q31) | File |
| Audio players | Play the body narration and, when picked, the intro and outro segments (Q17, Q30) | None |
| Download .mp3 | Saves the audio (Q1); container format `for: stack` | File |
| Re-run with voice | Regenerates audio with the picked voice (Q17) | Stays |
| Image grid, per image: Download, Regenerate | Saves one image; regenerates that one image (Q17) | File / stays |
| Download all | Saves every image and the thumbnail (Q1, Q17), one archive (assumed) | File |
| Re-run (images) | Regenerates the whole image stage (Q17) | Stays |
| Video player | Plays the slideshow with cards and alternating zoom (Q1, Q18) | None |
| Download .mp4 | Saves the video (Q1) | File |
| Re-render | Rebuilds the video from the current article, audio, images (Q17) | Stays |

## States

Per stage, one of: pending / running / done / failed (Q17), plus provided and skipped (assumed labels, Q28 and Off sources).
