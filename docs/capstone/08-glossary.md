...
| Document | The optional seventh stage: the article laid out locally as a styled PDF (`document_pdf`) in the Plain theme or a Library theme, with the thumbnail as its cover. `packages/app/src/slices/document/render.ts` | `slices/document`, `document:pdf` |
