...
Narration Preparation shows Use Documentary Starter on the Body label's own row, so switching Kind never moves the body. It fills only the unsaved body; replacing nonempty text requires confirmation. Cancel keeps the original text and Save remains explicit. The starter describes cue-only documentary delivery, not article rewriting (`packages/web/src/routes/prompt-editor.tsx`, `packages/web/src/lib/narration-starter.ts`). YouTube Description shows Use Built-in Starter in the same place, filling the body with the wording a project uses when it picks no Description prompt (`packages/app/src/slices/youtube/model.ts`).
