...
- Content groups title, article, keywords, text generation (help behind an InfoTip) and Research. Outputs groups Audio, Images, Thumbnail and Export rails with applicable sources and contextual provider/keyword links. The Export rail carries Seconds per image, Zoom (%) and a Motion select (Zoom in and out, Pan across, Mix of both, Still; help "How each image moves while it's on screen." behind an InfoTip) when a video is made and Silence at start and end when there is narration, each a short raw-text field with its help behind an InfoTip (`packages/web/src/play/stage-rails.tsx`). Below them the Export rail always carries the YouTube description checkbox (help behind an InfoTip) and a Description prompt select whose first option is Built-in; the prompt is disabled while the checkbox is off, the checkbox is disabled with the line "Needs narration." while narration is Off (`packages/web/src/play/youtube-description.tsx`). Rails reserve no gaps for empty rows (`packages/web/src/play/content-section.tsx`, `packages/web/src/play/outputs-section.tsx`).
...
Review's Outputs group adds a YouTube description row while narration is on: Off, or "<prompt> prompt · One LLM call after subtitle timing" (Built-in when none is picked); the Text generation provider row shows when it is on, and the estimate adds a YouTube description LLM row (`packages/web/src/play/review-summary.tsx`, `packages/app/src/slices/estimate/index.ts`).
