...
- **Rundown strip** (`project/navigation.tsx` `RundownStrip`): a Run cell with percent and the Overall progress bar, then five section cells, Article, Audio, Images, Video and Document (lamp, name, state word with live announcement, glyph and summary). Research and the thumbnail have no cell: they are shown inside Article and Images, and a companion that is running, failed or canceled lights its section's lamp and prefixes the summary ("Research: Stopped after 1 attempt"); Images switched off with a thumbnail on reads as the Thumbnail section (`project/sections.ts`). It is the stage navigation, the overall progress and the output summary at once; selecting a cell opens the Output tab on it. The cells scroll horizontally on narrow screens.
...
- **Output tab**: the active stage's block. Its header shows glyph, title, summary, progress text and state word over an always-drawn 2 px meter. A failed or canceled stage (the section's own or its companion's) adds one failure row: message, attempts, an Error details popover, the unready-provider line with Open Settings, an InfoTip with the recovery explanation, and a SplitButton "Retry stage" (for a companion "Retry research" or "Retry thumbnail") whose menu holds "Re-run section" (confirmation dialog; enabled when `canRerunSection`). Stage refusals show inside the stage's own block with Dismiss. The body contains article text, narration, images or export; sections without output show a centred explanation (`project/stage-row.tsx`, `project/bodies.tsx`). Article's tabs are Article, Research (when the run researches), Sources and Pronunciation; Research holds the notes, live writing while it runs, Re-run research and Copy research notes, and is open by default while research is running or stopped (`project/body-article.tsx`, `project/body-research.tsx`). Images opens with the thumbnail, when the run makes one, large under a Thumbnail heading with Regenerate thumbnail, Download thumbnail and Open folder and no prompt text, then the slideshow groups (`project/body-thumbnail.tsx`). When the project writes a YouTube description, the export body adds a YouTube part below the downloads, under a rule and an engraved heading rather than a box: Description and Tags as read-only text (placeholders say it is written after the subtitle timing), Copy description and Copy tags (disabled until the text exists), Download description.txt and Download tags.txt with Open folder, and a reserved StatusSlot that says "Copied the description." or why copying failed (`project/body-youtube.tsx`).
- **Edit tab**: the saved output status list (help behind the section InfoTip) and an ActionBar with Rebuild affected outputs and Edit project. Edit project replaces it with the form: a section sub-nav (Inputs, Article, Providers, Prompts, Subtitles, Images, Narration when audio generates, Captions, with count badges) beside one section (Inputs carries Silence gap, Silence at start and end when there is narration, and Seconds per image, Zoom (%) and a Motion select (Zoom in and out, Pan across, Mix of both, Still) when a video is made, each with a one-line hint; Prompts starts with the YouTube description checkbox and Description prompt select, Built-in dropping the frozen prompt copy); hidden sections stay mounted so uploads and unapplied edits survive a switch. A sticky ActionBar carries the status slot (newer revision available, waiting for uploads), the conflict reload action, Discard changes and Save changes (`project/revision-form.tsx`).
...
Download actions have adjacent Open folder actions for server-owned media, archives and subtitles. A pending request disables the action and reads Locating…. Native success opens the file manager. In managed Docker with the host helper installed, the helper opens the file manager as a native install would. Without the helper, success opens a popover with a labelled readonly host path in a status region, selected on focus, explaining that Docker can't open windows on the desktop and that running the Docker launcher again sets up the helper; errors show in the same popover. Paths are resolved from owned current/retained output records on the server, not supplied by the browser. The same component serves History, and downloads remain independent (`packages/web/src/project/open-folder.tsx`, `packages/web/src/project/revision-api.ts`, `packages/app/src/edge/http/folder-location.ts:13`).
