...
Start displays “Starting rebuild…” while its request is pending. Refusals and transport errors appear beside that button with field details and receive keyboard focus, bringing the error into view on narrow screens. The review and acknowledgements survive a retryable failure; stale previews close and focus the error by the revision actions. The same request identity is reused when retrying an unchanged request (`project/revision-feedback.tsx`, `project/revision-workspace.tsx`; 1.4.1 regression correction).
